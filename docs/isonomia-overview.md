...
Isonomia is a social platform built around a formal reasoning layer. The social layer — forums and feed, messaging and rooms, articles, shared libraries, user profiles — provides the space where communities gather. The reasoning layer — a deliberation engine grounded in formal argumentation theory, interactive proof theory, and evidence aggregation — provides the infrastructure that preserves what those gatherings produce. The two layers share a single data model and a continuous spectrum of formality: any conversation can become a structured deliberation, and every deliberation remains situated in a social context.

The platform is open-source, self-hostable, and designed as public infrastructure. There is no advertising, no algorithmic feed optimization, no data harvesting, and no engagement metrics. The data belongs to the community that produces it. The code is public. These commitments are enforced at the level of the architecture rather than the level of policy.
...
Isonomia exposes the public corpus as a crawlable, machine-citable search surface — "Google for arguments" — reachable identically by humans, search engines, and LLM agents.

**Consumer search page.** A server-rendered page at `/search/arguments?q=…` ([app/search/arguments/page.tsx](app/search/arguments/page.tsx)) renders ranked results as substrate-first cards: standing badge (`tested-survived`, `tested-attacked`, etc.), scheme chip, dialectical-fitness chip, hybrid-retrieval chip, attestation link, deep-link to counter-argument discovery, and lexical-coverage. The page emits `<link rel="alternate" type="application/json">` and `<link rel="alternate" type="application/ld+json">` so a single URL serves humans and crawlers. Honest-empty states distinguish no-intent / no-results / against-with-no-counters / API failure rather than collapsing them all into a generic "nothing found."

**Hybrid retrieval.** [`GET /api/v3/search/arguments`](app/api/v3/search/arguments/route.ts) is the single source of truth, called by the page, MCP, and external integrations. `mode=hybrid` (default for MCP, opt-in on the page) fuses pgvector cosine similarity with sparse lexical OR-token recall via Reciprocal Rank Fusion (K=60, per Cormack et al.); `mode=lexical` is deterministic substring matching; `mode=vector` is pure semantic. Each result carries an auditable `hybrid` block (`rrfScore`, `sparseRank`, `denseRank`, `denseDistance`).

**Quality filters.** Four filter params — `tested_only`, `min_cq_satisfied`, `min_evidence` (counts only evidence with `contentSha256`, i.e. provenance-anchored), and `since`/`until` ISO date range — narrow results to dialectically-tested, evidence-bearing arguments. Filters surface in the page UI as a collapsible "Quality filters" panel, in the OpenAPI spec, and as MCP tool params.

**Counter-citation discovery.** `include_strongest_counter=1` enriches each result with a `strongestCounter` block pointing at the most-engaged structural contester to its conclusion (rebut/undercut argument-edges + conflict applications, self-counters excluded by MOID). Honest-null when nothing is on file. The consumer page opts in by default so every visible card displays "Strongest counter: …" or "none on file" — there is no one-sided ranking.

**Stance retrieval.** [`GET /api/v3/claims/{moid}/stances`](app/api/v3/claims/[moid]/stances/route.ts) returns `{for: [...], against: [...]}` in one call — the killer query for any debate UI. "For" arguments conclude to the claim; "against" arguments are structural contesters. Both lists carry the full search-result shape so any client that understands a search result understands a stance result. The MCP tool `get_claim_stances` exposes the same. The claim page (`/c/{moid}`) renders the dual-column "Dialectical view" inline.

**SEO surface.** [`app/sitemap.ts`](app/sitemap.ts) emits a sitemap covering up to 40k argument permalinks + 8k claim permalinks + canonical scheme search topics; [`app/robots.ts`](app/robots.ts) allows `/a/`, `/c/`, `/search/arguments`, the v3 search API, and the discovery manifest while disallowing internal admin/cron/auth surfaces. [`app/api/og/search/arguments/route.tsx`](app/api/og/search/arguments/route.tsx) generates 1200×630 OG cards with live result count + sort/mode chips so a Slack or Twitter unfurl shows "12 results for ‘smartphones adolescent depression’" rather than a generic placeholder.
...
3. **Dialectical honesty by construction.** Citations ship with their opposition attached. `cite_argument.strongestObjection` is on by default; the public search API exposes the symmetric `include_strongest_counter` flag so every search result carries a `strongestCounter` block (or honest-null when none on file) pointing at the most-engaged structural contester to its conclusion. `standingState` is classified — `untested-default | untested-supported | tested-attacked | tested-undermined | tested-survived` — rather than collapsed to a raw float. `find_counterarguments`, the search route's `against` mode, and the strongest-counter helper all exclude same-conclusion-MOID self-counters by contract. `?sort=dialectical_fitness` re-ranks results by tested-and-survived status. `GET /api/v3/claims/{moid}/stances` and the matching `get_claim_stances` MCP tool return the dual `{for, against}` view in a single call.
4. **Standards-grounded interoperability.** AIF + JSON-LD + Schema.org + MCP, with no bespoke formats. AIF context publishes `aif:` / `as:` / `cq:` prefixes; the Schema.org composite combines `Claim` + `ScholarlyArticle` + `ClaimReview`. An OpenAPI 3.1 spec is served at `/api/v3/docs` (Scalar UI). The MCP server (24 tools, stdio) is installable into Claude Desktop in three lines, and groups its tools into argument-scope (read/write a single argument or claim), deliberation-scope (Track AI-EPI Pt. 4 readouts), and **ECC-scope** (typed categorical algebra over a deliberation — see Pillar 7).
...
7. **Typed categorical algebra, deterministic and graph-derived.** A typed implementation of Ambler 1996, *An Evidential Category of Claims*, exposed verbatim to LLM agents. Each claim has a typed `Hom(I, claim)` arrow; each derivation carries its assumption set; `confidence(arrow, monoid)` folds a closed-enum monoid (`min`, `product`, `ds`) over the arrow; `culpritSets()` answers the canonical demo question — *what would I have to retract to reject claim X?* — with a deterministic ranking (Ambler §4). MCP tools `ecc_arrow`, `ecc_culprits`, `ecc_confidence`, `ecc_enthymemes`, `ecc_transport`, `ecc_aggregate`, `ecc_evidential`, and `ecc_belief_revision_proposals` give an LLM client graph-derived answers with no LLM in the loop, and a Jest contract test asserts bit-identical JSON across repeated invocations on the same fixture. Three contracts the surface honors:
   - **Strict `isLogical`** (Ambler Def. 17): a derivation is logical only when every dependent assumption is `ACCEPTED` *and* the backing argument is HUMAN-authored (or AI/HYBRID + ratified).
   - **AI-flagged warrants are non-logical until ratified.** `propose_warrant` materializes the internal hom Λ: `[A, B] ≃ Hom(A ⊗ —, B)` as an AI-authored Argument plus an `AssumptionUse(role: 'warrant', status: 'PROPOSED')`; the resulting derivation never lifts to logical until a human ratifies the warrant text.
   - **One-hop cross-room transport.** `RoomTransportSnapshot` rows, written by the transport-aggregator worker on `RoomFunctor` upserts, carry one-hop transport only by contract — chained A→B→C transport is intentionally not supported, so an aggregated band `{ local, imported, total }` per claim is always auditable to a single source room.
