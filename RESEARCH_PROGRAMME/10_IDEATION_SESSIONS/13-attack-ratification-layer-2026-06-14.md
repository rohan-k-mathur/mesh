...
2. **Dev spec** — **SKELETON SEEDED 2026-06-14:** [`docs/ATTACK_RATIFICATION_DEV_SPEC.md`](../../docs/ATTACK_RATIFICATION_DEV_SPEC.md). Covers the `ConflictApplication` lifecycle enum + `ConflictRatification` table + `DeliberationPref` policy field (schema delta); the enforcement filter (with a migration/backfill that sets existing CAs `EFFECTIVE` so nothing silently disappears); the sign-off API + self-ratification/eligibility guards; the pending-state UI + notification wiring; and the recompute/revocation flow. Section bodies are stubs to flesh out.
