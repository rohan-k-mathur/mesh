...
Author: (your name / handle)Contact: (email / URL)
Version: v0.1Date: 2025-12-16Status: Draft
...
DISCUSSIONS_AND_CHAT_ARCHITECTU…
Mesh is implemented as an ecosystem of interacting surfaces—publishing, library, discussions, deliberation, and discovery—sharing a common substrate. Published articles appear as discoverable posts in a social feed, while also serving as hosts for embedded deliberation.
ARTICLE_SYSTEM_ARCHITECTURE
Sources and citations are deduplicated and resolved through a fingerprinted pipeline, and citations attach polymorphically to multiple reasoning objects.
STACKS_LIBRARY_SYSTEM_ARCHITECT…
Deliberations connect into a meta-level network (“Plexus”) that visualizes cross-references and supports argument transport with provenance.
AGORA_SUBSYSTEMS_ARCHITECTURE
...
* Publishing (Articles): Longform writing that can participate in the same discourse substrate as discussion and deliberation. When an article is published, it appears as a discoverable post in the home feed (a social distribution surface), rather than living outside the discourse graph. ARTICLE_SYSTEM_ARCHITECTUREThe feed includes a first-class ARTICLE post type and structured payload for distribution and preview. ARTICLE_SYSTEM_ARCHITECTUREARTICLE_SYSTEM_ARCHITECTURE
* Discussions (Chat + Forum): A dual-mode container in which participants can switch between real-time chat and threaded forum discussion, while retaining the ability to upgrade to formal deliberation. DISCUSSIONS_AND_CHAT_ARCHITECTU…
* Library (Stacks + Sources + Citations): A system for collecting, deduplicating, and citing sources across discourse objects. Sources are resolved via a normalization + fingerprint pipeline, preventing duplicates and enabling consistent attribution. STACKS_LIBRARY_SYSTEM_ARCHITECT…STACKS_LIBRARY_SYSTEM_ARCHITECT…Citations attach polymorphically to target entities (e.g., claim, argument, comment, move), allowing evidence to remain connected as reasoning is transformed. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Deliberation (Agora): A formalization target: a deliberation can be spawned from multiple host contexts and produces structured artifacts (e.g., debate sheets) suitable for evaluation, export, and reuse. In the implemented system, a deliberation may be created for a given hostType/hostId pair and can spawn an AgoraRoom and an associated DebateSheet. ARTICLE_SYSTEM_ARCHITECTUREARTICLE_SYSTEM_ARCHITECTURE
* Discovery + Cross-Deliberation Network (Plexus): A meta-level network connecting deliberations via cross-references, overlaps, and imports, supporting explicit transport of arguments between rooms. AGORA_SUBSYSTEMS_ARCHITECTUREIn the current implementation, cross-deliberation transport is expressed via a RoomFunctor model with claim mapping, preview/apply flows, fingerprint idempotency, and provenance tracking. CATEGORICAL_FOUNDATIONS_ARCHITE…
...
This is not a separate “mode” that requires moving to another platform. For example, the Discussions system is explicitly designed so discussions can upgrade into deliberations when complexity warrants.
DISCUSSIONS_AND_CHAT_ARCHITECTU…
Likewise, longform publishing and the library system both integrate directly with deliberation creation and evidence attachment pathways.
ARTICLE_SYSTEM_ARCHITECTURE
STACKS_LIBRARY_SYSTEM_ARCHITECT…
...
In the library subsystem, sources can originate from URLs, DOIs, or internal library posts, are normalized and fingerprinted for deduplication, and can be rated/annotated over time.
STACKS_LIBRARY_SYSTEM_ARCHITECT…
STACKS_LIBRARY_SYSTEM_ARCHITECT…
Citations attach to many target types via a polymorphic targetType/targetId pattern, allowing the same evidence graph to remain valid as discourse is transformed into claims, arguments, and dialogue moves.
STACKS_LIBRARY_SYSTEM_ARCHITECT…
...
* route the user into the deliberation panel
...
6. Package B: Publishing-Native — Articles (“Substack, but discourse is structured”)
6.1 Problem framing
...
* Article: a longform document with a draft→publish lifecycle and revision history. ARTICLE_SYSTEM_ARCHITECTURE
* Template: a layout preset (“standard”, “feature”, etc.) that affects rendering but not the underlying reasoning substrate. ARTICLE_SYSTEM_ARCHITECTURE
* Anchor / Thread: DOM-anchored comment threads attached to text selections. ARTICLE_SYSTEM_ARCHITECTUREARTICLE_SYSTEM_ARCHITECTURE
* Deliberation host: each article is “deliberation-native” and serves as a host surface for structured discourse. ARTICLE_SYSTEM_ARCHITECTURE
* FeedPost (ARTICLE type): a published article can be represented as a feed object for distribution/discovery. ARTICLE_SYSTEM_ARCHITECTURE
...
7. Create a feed post (optional) ARTICLE_SYSTEM_ARCHITECTURE
...
ARTICLE_SYSTEM_ARCHITECTURE
The system description explicitly frames this as social-feed discovery analogous to “Substack/Bluesky,” while still treating it as infrastructure rather than marketing.
ARTICLE_SYSTEM_ARCHITECTURE
6.5 Outputs and artifacts
Publishing produces durable artifacts beyond the comment stream:
* Published article page (server-rendered for performance/SEO) ARTICLE_SYSTEM_ARCHITECTURE
* Revision snapshots created at publication time (and potentially elsewhere in the workflow) ARTICLE_SYSTEM_ARCHITECTURE
* Anchored annotation threads that remain attached to specific spans of the document ARTICLE_SYSTEM_ARCHITECTURE
* Feed posts that allow distribution and discovery (ARTICLE-type) ARTICLE_SYSTEM_ARCHITECTURE
* A deliberation host context: the article provides a stable “place” where informal commentary can be progressively formalized into claims/arguments as needed. ARTICLE_SYSTEM_ARCHITECTURE
6.6 Integration points with the shared substrate
* Deliberation-native hosting is a design principle of the article system (“Articles are first-class deliberation hosts”). ARTICLE_SYSTEM_ARCHITECTURE
* Portable content representation (TipTap JSON AST) makes it feasible to treat publishing artifacts as composable inputs/outputs within the broader platform. ARTICLE_SYSTEM_ARCHITECTURE
* Feed integration provides a distribution mechanism that does not sever provenance: the feed object points back to the canonical article entity. ARTICLE_SYSTEM_ARCHITECTURE
...
7. Package C: Curation- and Evidence-Native — Stacks & Library (“Are.na, but sources are executable”)
7.1 Problem framing
...
* Stack: a curated collection. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* LibraryPost: an individual document item (e.g., a PDF) with metadata and thumbnails. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Source: a canonical reference entity (URL, DOI, or LibraryPost-backed). STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Citation: a link from a Source to a target entity (e.g., comment, claim, argument). STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Deliberation host type (library_stack): stacks can host deliberations directly. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Cross-deliberation reference edges (StackReference model) to support knowledge graph connections across contexts. STACKS_LIBRARY_SYSTEM_ARCHITECT…
Architecturally, the system also commits to:
* Polymorphic citations: “Single Citation model targets multiple entity types.” STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Source deduplication via SHA1 fingerprinting. STACKS_LIBRARY_SYSTEM_ARCHITECT…
...
1. Upload: user uploads into a stack, creating a LibraryPost. STACKS_LIBRARY_SYSTEM_ARCHITECT…
2. Organize: user reorders/curates within the stack. STACKS_LIBRARY_SYSTEM_ARCHITECT…
3. Discuss: user comments in a FeedPost-based thread attached to the stack. STACKS_LIBRARY_SYSTEM_ARCHITECT…STACKS_LIBRARY_SYSTEM_ARCHITECT…
4. Cite: citations resolve to canonical Sources and attach to targets. STACKS_LIBRARY_SYSTEM_ARCHITECT…
5. Lift: a comment can be promoted (“lifted”) into the deliberation system. STACKS_LIBRARY_SYSTEM_ARCHITECT…STACKS_LIBRARY_SYSTEM_ARCHITECT…
6. Deliberate: deep dive deliberation UI consumes the EvidenceList aggregation endpoint. STACKS_LIBRARY_SYSTEM_ARCHITECT…
7. Share: a stack can be represented as a feed post for distribution/discovery. STACKS_LIBRARY_SYSTEM_ARCHITECT…
7.4.1 Citation resolution (deduplication by construction)
To avoid source fragmentation, Mesh computes a fingerprint over normalized DOI/URL/library identifiers and uses SHA1 to deduplicate sources.
STACKS_LIBRARY_SYSTEM_ARCHITECT…
URLs are canonicalized (e.g., stripping fragments and common tracking parameters).
STACKS_LIBRARY_SYSTEM_ARCHITECT…
...
* Curated stacks of source documents (not just links) STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Canonical Sources with deduplication and normalization STACKS_LIBRARY_SYSTEM_ARCHITECT…STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Polymorphic citations that can attach to multiple discourse entities STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Evidence aggregates that track usage and can support quality/rating signals (“Track source usage and quality ratings across deliberations”). STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Deliberation hosts via library_stack, enabling a stack to become the locus of structured debate rather than merely a reading list. STACKS_LIBRARY_SYSTEM_ARCHITECT…
7.6 Integration points with the shared substrate
* Deliberation hosting is explicit: stacks can host deliberations via the library_stack host type. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Lift-to-debate provides a concrete upgrade path from informal discussion into structured claims. STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Feed integration allows stacks/library objects to appear as shareable/discoverable posts (“Stack → FeedPost (LIBRARY) → LibraryCard in feed”). STACKS_LIBRARY_SYSTEM_ARCHITECT…
* Event-driven updates support real-time synchronization, reinforcing that the library is not static storage but a live substrate for discourse. STACKS_LIBRARY_SYSTEM_ARCHITECT…
...
Below are draft sections for Package D and Package E, written to slot directly into the canonical whitepaper (Bitcoin-style: systems-first, minimal marketing, clear “problem → what is needed → what we propose” motion).

7. Package D — Discussions and Chat
7.1 Problem framing
...
AGORA_DELIBERATION_SYSTEM_ARCHI…
7.3 Core objects
...
* DiscussionView: a unified container presenting two panes:
    ...
    * ChatPane for real-time messages ARTICLE_SYSTEM_ARCHITECTURE
...
1. Threaded exchange (forum mode)Participants write longer posts, reply in threads, and build structured context over time in ForumPane. ARTICLE_SYSTEM_ARCHITECTURE
2. High-tempo exchange (chat mode)Participants coordinate via ChatPane; messages are broadcast in real-time to a discussion-scoped channel, while post updates are also broadcast to keep all clients coherent. ARTICLE_SYSTEM_ARCHITECTURE
3. Cross-mode repliesReplies can target different object types via ReplyTarget, allowing the conversation graph to reflect actual conversational structure rather than being constrained by whether the author happened to be “in chat” or “in posts.”
4. Drift creation and resolutionWhen a subtopic threatens to derail a thread, a participant can spawn a drift: a side-thread that captures the divergence without losing the anchor to the original context, and can later be resolved and summarized back. DISCUSSIONS_AND_CHAT_ARCHITECTU…
5. Upgrade to deliberationWhen the group needs stronger structure (e.g., evidence tracking, canonical claims, explicit challenges), the discussion can be upgraded: the UI exposes a “Deliberate” action that opens/creates a deliberation for the same host context, and the discussion becomes the informal layer inside that deliberation. AGORA_DELIBERATION_SYSTEM_ARCHI…
7.5 Outputs and artifacts
...
7.8 Bridge: From Discussion to Deliberation
...
ARGUMENT_CHAIN_SYSTEM_DEEP_REVI…
...
8. Package E — Agora: Formal Deliberation as a Public Utility
8.1 Problem framing
...
* and connect the evolving structure of disagreement to durable outcomes that can be cited and audited. AGORA_DELIBERATION_SYSTEM_ARCHI…
...
* Structured claim management with stable identifiers and typed relationships (including distinctions among how something is challenged). AGORA_DELIBERATION_SYSTEM_ARCHI…
* Scheme-based argumentation (Walton-style schemes) with critical questions surfaced as explicit vulnerability points. AGORA_DELIBERATION_SYSTEM_ARCHI…
* Dialogue protocol enforcement: assertions, challenges, concessions, and retractions are first-class moves with provenance. AGORA_DELIBERATION_SYSTEM_ARCHI…
* Composable outputs: deliberations generate thesis documents, KB pages, and exportable graphs as institutional memory. AGORA_DELIBERATION_SYSTEM_ARCHI…
...
1. Claims & Evidence: propositions, claims, claim edges, evidence links. AGORA_DELIBERATION_SYSTEM_ARCHI…
2. Arguments & Dialogue: arguments, schemes, critical questions, dialogue moves, commitments, chains. AGORA_DELIBERATION_SYSTEM_ARCHI…
3. Outputs & Artifacts: thesis documents, debate sheets, KB pages, briefs, and related compositional artifacts. AGORA_DELIBERATION_SYSTEM_ARCHI…
...
12. View analytics
13. Manage issues/objections AGORA_DELIBERATION_SYSTEM_ARCHI…
...
* DM-nodes represent dialogue moves. AGORA_DELIBERATION_SYSTEM_ARCHI…
...
* Glossary terms (local definition layer). AGORA_DELIBERATION_SYSTEM_ARCHI…
...
AGORA_DELIBERATION_SYSTEM_ARCHI…
8.8 Interfaces as system architecture
...
* AIF scheme/graph operations. AGORA_DELIBERATION_SYSTEM_ARCHI…
...
9. Package F — Mesh Plexus: A Meta-Network of Deliberations for Transportable Discourse Artifacts
9.1 Problem framing
...
CATEGORICAL_FOUNDATIONS_ARCHITE…
CATEGORICAL_FOUNDATIONS_ARCHITE…
9.3 Core objects
At the Plexus layer, the essential primitives are:
(a) Network nodes and edges.The Network API returns a set of room nodes and typed edges (a “meta-edge” graph).
CATEGORICAL_FOUNDATIONS_ARCHITE…
* RoomNode describes a deliberation room with summary metrics (counts, acceptance distribution, tags, updatedAt). AGORA_SUBSYSTEMS_ARCHITECTURE
* MetaEdge links two rooms with a kind and weight. AGORA_SUBSYSTEMS_ARCHITECTURE
The edge kinds are a closed set:
* xref, overlap, stack_ref, imports, shared_author. CATEGORICAL_FOUNDATIONS_ARCHITE…
(b) Five edge types (semantics).Each edge kind corresponds to a concrete model/source of truth:
CATEGORICAL_FOUNDATIONS_ARCHITE…
...
(c) Transport primitives.Transport is implemented via:
* RoomFunctor: an object-level mapping from claims in room A to claims in room B (claimMapJson), with a unique pair constraint on (fromRoomId, toRoomId). CATEGORICAL_FOUNDATIONS_ARCHITE…
* ArgumentImport: a provenance-carrying record of an imported argument (including a unique SHA-1 fingerprint for idempotency). CATEGORICAL_FOUNDATIONS_ARCHITE…
...
9.4.1 Discover and inspect the network
...
* or initiate transport (“copy arguments between rooms”). AGORA_SUBSYSTEMS_ARCHITECTURE
9.4.2 Link rooms
...
AGORA_SUBSYSTEMS_ARCHITECTURE
CATEGORICAL_FOUNDATIONS_ARCHITE…
9.4.3 Transport arguments (A → B)
Transport is implemented as an explicit workflow backed by stable IDs and idempotent imports:
1. Map claims (A → B).The user establishes (or edits) a mapping between claims across deliberations, stored in RoomFunctor.claimMap_toggle. CATEGORICAL_FOUNDATIONS_ARCHITE…
2. Preview import proposals.The preview endpoint returns proposed imports with fingerprints (SHA-1) to make the import operation idempotent. CATEGORICAL_FOUNDATIONS_ARCHITE…
3. Apply (materialize) imports.The apply endpoint materializes selected imports in a transaction: it checks the fingerprint, creates the imported argument/support if needed, and records provenance in provenanceJson. CATEGORICAL_FOUNDATIONS_ARCHITE…
...
9.4.4 Virtual vs. materialized imports
...
CATEGORICAL_FOUNDATIONS_ARCHITE…
9.5 Outputs and artifacts
Plexus produces durable cross-deliberation artifacts:
* A queryable room graph: (rooms, edges) returned from /api/agora/network, suitable for visualization and filtering. CATEGORICAL_FOUNDATIONS_ARCHITE…
* Typed relationships that preserve semantics of “how these rooms are related” (xref vs overlap vs imports vs stack_ref). CATEGORICAL_FOUNDATIONS_ARCHITE…
* Transport records: RoomFunctor mappings and ArgumentImport rows that preserve provenance and enforce idempotency via fingerprint. CATEGORICAL_FOUNDATIONS_ARCHITE…CATEGORICAL_FOUNDATIONS_ARCHITE…
...
* Stacks and evidence contribute stack_ref edges via StackReference, connecting reading/curation to debate reuse. CATEGORICAL_FOUNDATIONS_ARCHITE…
* Deliberation evaluation informs room-level metrics (e.g., acceptance distribution in RoomNode), allowing discovery and navigation to be based on reasoning structure, not only engagement. AGORA_SUBSYSTEMS_ARCHITECTURE
* Transport turns deliberations into a knowledge commons: imported arguments are visible to the deliberation engine and can be evaluated under local premises and preferences (materialized or virtual). CATEGORICAL_FOUNDATIONS_ARCHITE…
9.7 Implementation notes
* The Plexus layer is backed by a dedicated network API (/api/agora/network) and a typed edge vocabulary. CATEGORICAL_FOUNDATIONS_ARCHITE…
* The UI supports multiple views (graph/board/matrix) and exposes explicit actions for link creation and transport initiation. AGORA_SUBSYSTEMS_ARCHITECTURE
* Transport is implemented as a small, explicit API suite (/api/room-functor/*) supporting mapping, preview, and apply/materialize. CATEGORICAL_FOUNDATIONS_ARCHITE…
* Current implementation status includes fully working claim mapping, preview proposals, materialized and virtual imports, and fingerprint idempotency; composition of transport functors (A→B→C) is explicitly tracked as future work. CATEGORICAL_FOUNDATIONS_ARCHITE…
...
3. Discourse does not compound.Mesh introduces a cross-deliberation network (Plexus) and transport mechanisms (RoomFunctor + ArgumentImport) that allow arguments to move between deliberations with provenance preserved. CATEGORICAL_FOUNDATIONS_ARCHITE…CATEGORICAL_FOUNDATIONS_ARCHITE…
