/**
 * GET /api/v3/deliberations/[id]/ecc/aggregate?claimId=...&mode=min|product
 *
 ...
 * computed under the noisy-OR (product) reducer; DS would need its own
 * cached payload shape, which is out of scope for Sprint E.
 */
...
import { evaluateConfidence, loadClaimArrow } from "@/lib/argumentation/eccLoader";

export const dynamic = "force-dynamic";

const ALLOWED = new Set(["min", "product"]);
...
export async function GET(
  ...
  const mode = (url.searchParams.get("mode") ?? "product").toLowerCase();
  if (!claimId) {
  ...
  if (!ALLOWED.has(mode)) {
    return NextResponse.json(
      { ok: false, error: "mode must be one of: min, product (ds aggregation not cached)" },
      { status: 400 },
    );
  }
  ...
  const localValue =
    loaded ? (evaluateConfidence(loaded, mode as "min" | "product") as number | null) : null;
  ...
  for (const s of snaps) {
    ...
    importedContributions.push({
      ...
      score: reduceImportedScores(sourceScores, mode as "min" | "product"),
    });
  }

  const importedScores = importedContributions.map((c) => c.score);
  const imported = reduceImportedScores(importedScores, mode as "min" | "product");
  const local = localValue ?? 0;
  const total = combineLocalAndImported(local, imported, mode as "min" | "product");
