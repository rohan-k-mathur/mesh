...
import { declineHandoff } from "@/lib/facilitation/handoffService";
...
export async function POST(
  ...
  try {
    const handoff = await declineHandoff({
      handoffId: params.id,
      actorId: auth.userId,
    ...
    if (/handoff target mismatch/i.test(msg)) return apiError("FORBIDDEN", msg);
