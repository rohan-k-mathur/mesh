...
export async function GET(
  ...
  try {
    ...
    const result = await exportClusterFamily(params.clusterTag, {
      format,
      includeHierarchy,
      includeCQs,
    ...
    if (!result.success) {
      return NextResponse.json(
        ...
        { status: result.error?.code === "CLUSTER_NOT_FOUND" ? 404 : 500 }
      );
    }
    ...
    if (download) {
      headers["Content-Disposition"] = `attachment; filename="${params.clusterTag}.${ext}"`;
    }
    ...
    console.error("AIF cluster export error:", error);
    return NextResponse.json(
