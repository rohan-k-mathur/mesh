...
export default function SequentBadge({
  ...
  const color = summary.verdict === 'strong' ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
  ...
  return (
    <> <button onClick={onClick} className={`text-[11px] px-1.5 py-0.5 rounded border ${color}`}
    title={`min entail=${summary.perDelta.length ? Math.min(...summary.perDelta.map(d=>d.entailMax)).toFixed(2) : '0'}; max contra=${summary.perDelta.length ? Math.max(...summary.perDelta.map(d=>d.contraMax)).toFixed(2) : '0'}`}>
    Entailment: {summary.verdict}
  </button></>
    // <button
    //   type="button"
