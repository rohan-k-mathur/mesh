...
function escape(s: string) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
 function reFor(words: ReadonlyArray<string>) {
  ...
  const sorted = Array.from(words).sort((a,b)=>b.length-a.length).map(escape);
...
export const LEX_EMOTION = {
    positive: [
      'benefit','improve','progress','opportunity','thrive','success','boost','effective','robust',
      'fair','just','equitable' // can move to fairness frame if it causes FP in your corpus
    ],
    negative: [
      'harm','risk','damage','collapse','crisis','threat','failure','costly','burden','danger','unsafe'
    ],
  };
...
export const LEX_FRAMES = {
    economic: [
      'economic growth','job creation','productivity','market impact','budget deficit','cost-benefit',
      'tax burden','fiscal responsibility'
    ],
    morality: [
      'right and wrong','moral duty','ethical obligation','moral obligation','virtue','integrity'
    ],
    security: [
      'national security','public safety','homeland security','border security','security threat'
    ],
    fairness: [
      'equal opportunity','equal treatment','level playing field','social justice','fair share'
    ],
    capacity: [
      'resources','infrastructure capacity','staffing shortages','administrative burden','implementation capacity'
    ],
  };
  ...
  function excludeInside(text: string, hits: Hit[], mask: RegExp) {
    ...
    const rx = new RegExp(mask.source, mask.flags.includes('g') ? mask.flags : mask.flags + 'g');
...
export type LexKind =
  | { family: 'emotion'; tag: 'positive' | 'negative' | 'joy' | 'anger' | 'fear' | 'sadness' }
  | { family: 'frames'; tag: 'economic' | 'morality' | 'security' | 'fairness' | 'capacity' }
  | { family: 'liwc'; tag: 'certainty' | 'tentative' | 'negation' | 'pronoun-first' | 'pronoun-second' }
 ...
 const LOGOS: Record<LogosTag, string[]> = {
   evidence: [
     'evidence','dataset','data set','data','study','trial','randomized','meta-analysis','systematic review',
     'peer[- ]reviewed','replicate','replication','sample size','cohort','placebo','control group',
     'p-value','confidence interval','effect size','statistically significant','significant at','regression','correlation',
     'model','baseline','variance','standard deviation','odds ratio','hazard ratio'
   ],
   quant: [
     'percent','percentage','proportion','ratio','median','mean','average','quartile','decile',
     'n=','N=','σ','μ','CI','95% CI','R²','p <','p ≤','p=','p≤'
   ],
   logic: [
     'therefore','thus','hence','implies','it follows','infer','deduce','premise','conclusion','contradiction'
   ],
   method: [
     'methodology','protocol','procedure','pre-registered','pre registered','blinded','double[- ]blind','placebo[- ]controlled',
     'power calculation','outcome measure','endpoint','instrument','operationalize','operational definition'
   ],
 };
 ...
 const ETHOS: Record<EthosTag, string[]> = {
   credentials: [
     'PhD','M\\.?D\\.?','J\\.?D\\.?','DPhil','DSc','MSc','MBA','CPA','Esq\\.?','professor','prof\\.?','dr\\.?','licensed','certified','accredited'
   ],
   duty: [
     'fiduciary','duty of care','professional standard','best practice','code of ethics','oath','pledge'
   ],
   integrity: [
     'transparent','disclose','disclosure','conflict of interest','independent','impartial','nonpartisan','bipartisan','reputable','trustworthy','credible','reliability','track record'
   ],
   collective: [
     'on behalf of','in good faith','as a team','our responsibility','shared responsibility','consensus','committee','board','peer committee'
   ],
 };
...
const makeLoose = (phrases: string[]) =>
 ...
 type ScanOptions = {
   includeFamilies?: { pathos?: boolean; frames?: boolean; liwc?: boolean; logos?: boolean; ethos?: boolean; };
 ...
 const invertMasked = (text: string, masks: RegExp[]): Range[] => {
  ...
  export function scanLexiconHits(text: string, opts: ScanOptions = {}): Hit[] {
    ...
    let hits: Hit[] = [];
       const {
         includeFamilies = { pathos: true, frames: true, liwc: true, logos: true, ethos: true },
    ...
    //            if (includeFamilies.ethos)  for (const { re } of ETHOS_PATTERNS) scanOverCat(re, 'ethos');
               ...
               if (includeFamilies.logos)  for (const { re } of LOGOS_PATTERNS) scanOver(re, 'logos');
  if (includeFamilies.ethos)  for (const { re } of ETHOS_PATTERNS) scanOver(re, 'ethos');
        ...
        export function scanLexiconHitsDetailed(text: string, opts: ScanOptions = {}): LexHit[] {
          const { includeFamilies = { logos: true, ethos: true }, maskPatterns = DEFAULT_MASKS, maxHitsPerFamily = 500 } = opts;
          const hits: LexHit[] = [];
          const allow = invertMasked(text || '', maskPatterns);
          ...
          if (includeFamilies.logos)  for (const { tag, re } of LOGOS_PATTERNS) scanF(re, { family: 'logos', tag });
          if (includeFamilies.ethos)  for (const { tag, re } of ETHOS_PATTERNS) scanF(re, { family: 'ethos', tag });
          return hits.sort((a, b) => a.start - b.start || a.end - b.end);
        }
...
export function analyzeLexiconsMany(texts: string[]): LexiconSummary {
  ...
  const valenceScorePer100w = Math.round(((affectCounts.positive - affectCounts.negative) / words) * 10000) / 100;
  ...
  return { words, affectCounts, valenceScorePer100w, frameCounts, topFrames, liwcCounts };
}
