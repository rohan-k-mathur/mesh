...
type Arg = {
      id: string; text: string; confidence?: number|null; createdAt: string; authorId: string;
...
export default function ArgumentsList({
  ...
  if (loading) return <div className="text-xs text-neutral-500">Loading arguments…</div>;
  ...
  {a.modality && <span className="px-1.5 py-0.5 border rounded">{a.modality}</span>}
  {a.mediaType && a.mediaType!=='text' && <span className="px-1.5 py-0.5 border rounded">{a.mediaType}</span>}
</div>
            <div className="mt-2 flex gap-2">
              <button className="px-2 py-1 border rounded" onClick={() => onReplyTo(a.id)}>Reply</button>
              <button className="px-2 py-1 border rounded" onClick={() => approve(a.id, true)}>Approve</button>
              <button className="px-2 py-1 border rounded" onClick={() => approve(a.id, false)}>Unapprove</button>
                {a.mediaType === 'image' && (
    <>
      <button className="px-2 py-1 border rounded"
        onClick={async()=>{
          await fetch(`/api/deliberations/${deliberationId}/issues`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ label: 'Image – Appropriateness', links: [a.id] })
          }); alert('Opened issue: Image – Appropriateness');
        }}>Dispute image (Appropriateness)</button>
      <button className="px-2 py-1 border rounded"
        onClick={async()=>{
          await fetch(`/api/deliberations/${deliberationId}/issues`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ label: 'Image – Depiction', links: [a.id] })
          }); alert('Opened issue: Image – Depiction');
        }}>Dispute image (Depiction)</button>
    </>
  )}
            </div>
          </li>
