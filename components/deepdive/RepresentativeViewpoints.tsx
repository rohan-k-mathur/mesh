...
export function RepresentativeViewpoints(props: {
  ...
  const [argsMap, setArgsMap] = useState<Record<string,string>>({});
   ...
   useEffect(()=> {
     (async()=>{
       const res = await fetch(`/api/deliberations/${props.deliberationId}/arguments`);
       if (!res.ok) return;
       const data = await res.json();
       const map: Record<string,string> = {};
       (data.arguments ?? []).forEach((a:any)=> { map[a.id]=a.text; });
       setArgsMap(map);
     })();
  ...
  return (
    <div className="rounded-md border p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium">Representative viewpoints (k={s.k})</div>
        <div className="text-xs text-neutral-600 flex items-center gap-3">
          ...
          {s.rule === 'maxcov' && s.jrSatisfied && <span className="ml-2 px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border">JR</span>}
        </div>
      </div>
        ...
        <div className="text-[11px] text-neutral-600">
          ...
          {s.conflictsTopPairs.slice(0,3).map((p,i)=>(
            <span key={i} className="mr-2">
              “{(argsMap[p.a] ?? p.a).slice(0,38)}” × “{(argsMap[p.b] ?? p.b).slice(0,38)}” ({p.count})
            </span>
          ))}
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <button className="text-xs underline" onClick={() => props.onReselect?.(s.rule)}>Recompute</button>
