...
import ForumPane from "./ForumPaneGlass";
...
const canEditTitle = me && discussion?.createdById && String(discussion.createdById) === String(me);
  ...
  const MAX_DESC_LENGTH = 34;
  const descText = discussion.description?.trim() || '';
  const needsTruncation = descText.length > MAX_DESC_LENGTH;
  const displayDescription = !showFullDescription && needsTruncation 
    ? descText.slice(0, MAX_DESC_LENGTH) + '...' 
    : descText;
