...
const KLBL: Record<EdgeKind,string> = {
  xref:'xref', overlap:'overlap', stack_ref:'stack', imports:'imports', shared_author:'authors'
};
...
function Dot({ color }: { color: string }) {
  return <span className="inline-block w-2 h-2 rounded" style={{ background: color }} />;
}
function Chip({ color, label, value }: { color:string; label:string; value:number }) {
  return (
    <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-[2px] rounded border"
...
function Donut({
  r = 18, acc, rej, und, tauShare, selected
}: { r?: number; acc:number; rej:number; und:number; tauShare?:number|undefined|null; selected?:boolean }) {
  const L = (rad:number) => 2*Math.PI*rad;
  const Ring = ({rad, col, w, share}:{rad:number; col:string; w:number; share:number}) => (
    <circle r={rad} stroke={col} strokeWidth={w}
            strokeDasharray={`${L(rad)*share} ${L(rad)}`} transform="rotate(-90)" fill="none" />
  );
  return (
    <svg width={(r+20)*2} height={(r+20)*2} viewBox={`${-(r+20)} ${-(r+20)} ${(r+20)*2} ${(r+20)*2}`}
         aria-hidden="true">
      <circle r={r} fill={selected ? '#10b981' : '#0ea5e9'}
              fillOpacity={selected ? 0.95 : 0.85}
              stroke={selected ? '#065f46' : '#0369a1'} strokeWidth={selected ? 2 : 1}/>
      <Ring rad={r+4}  col="#10b981" w={3} share={acc} />
      <Ring rad={r+8}  col="#f43f5e" w={2} share={rej} />
      <Ring rad={r+12} col="#94a3b8" w={2} share={und} />
      {typeof tauShare === 'number' && <Ring rad={r+16} col="#7c3aed" w={2} share={tauShare} />}
    </svg>
  );
}
...
export default function PlexusBoard({
  scope = 'public',
  onLinkCreated,
}: { scope?: 'public'|'following'; onLinkCreated?: () => void; }) {
  const { data, error, mutate } = useSWR<Network>(`/api/agora/network?scope=${scope}`, fetcher, { revalidateOnFocus: false });
  ...
  const [orderBy, setOrderBy] = usePersistentState<OrderBy>('plexus:board:order', 'recent');
  const [sel, setSel] = usePersistentState<string|null>('plexus:board:sel', null);
  const [source, setSource] = usePersistentState<string|null>('plexus:board:source', null);

  const [linkMode, setLinkMode] = usePersistentState<boolean>('plexus:board:linkMode', false);
  const [linkAction, setLinkAction] = usePersistentState<SketchAction>('plexus:board:linkAction', 'transport');
  ...
  function inc(id:string, k:EdgeKind) { const cur = counts.get(id) ?? { xref:0, overlap:0, stack_ref:0, imports:0, shared_author:0 }; cur[k]++; counts.set(id, cur); }
  for (const e of edges) { inc(e.from, e.kind); inc(e.to, e.kind); }

  // Tags
  const allTags = Array.from(new Set(rooms.flatMap(r => r.tags ?? []))).sort();
  const [tagFilter, setTagFilter] = usePersistentState<string[]>('plexus:board:tags', []);
  const toggleTag = (t:string) => setTagFilter(xs => xs.includes(t) ? xs.filter(y=>y!==t) : [...xs, t]);


  // Search + filter + order
  const qn = q.trim().toLowerCase();
  const filtered = rooms
    .filter(r => !tagFilter.length || (r.tags ?? []).some(t => tagFilter.includes(t)))
  ...
  // Neighborhood (1‑hop)
  const neighborsOf = React.useCallback((id:string) => {
    const out = new Map<string, { kinds:Set<EdgeKind>, weights:number[] }>();
    for (const e of edges) {
      if (e.from===id) { const m = out.get(e.to) ?? { kinds:new Set<EdgeKind>(), weights:[] }; m.kinds.add(e.kind); m.weights.push(e.weight); out.set(e.to, m); }
      else if (e.to===id) { const m = out.get(e.from) ?? { kinds:new Set<EdgeKind>(), weights:[] }; m.kinds.add(e.kind); m.weights.push(e.weight); out.set(e.from, m); }
    }
    return Array.from(out.entries()).map(([rid, meta]) => ({
      id: rid, kinds: Array.from(meta.kinds), weight: meta.weights.reduce((a,b)=>a+b,0),
    ...
    <div className="rounded-xl border bg-white/80 p-3">
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-slate-600">• links {edges.length}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-2 py-1 text-[12px] rounded border bg-white/70" aria-label="Order"
          >
          ...
          <div className="hidden sm:flex items-center gap-2 pl-2 ml-2 border-l">
            ...
            <select
              value={linkAction} onChange={(e)=>setLinkAction(e.target.value as SketchAction)}
              className="px-2 py-1 text-[12px] rounded border bg-white/70" aria-label="Link action"
          ...
          {allTags.map(t => (
            <button key={t}
              className={clsx(
                ...
                tagFilter.includes(t) ? 'bg-slate-900 text-white border-slate-900' : 'bg-white hover:bg-slate-50'
              )}
              onClick={()=>toggleTag(t)}
            >{t}</button>
          ))}
          {tagFilter.length>0 && (
            <button className="text-[11px] underline ml-2" onClick={()=>setTagFilter([])}>clear</button>
          )}
        </div>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filtered.map(r => {
          ...
          const acc = r.accepted/t, rej=r.rejected/t, und=r.undecided/t;
          const c = counts.get(r.id) ?? { xref:0, overlap:0, stack_ref:0, imports:0, shared_author:0 };
          ...
          return (
            <div key={r.id}
                 ...
                 onMouseEnter={() => { prefetch(r.id); if (tau!=null && gated===undefined) fetchGated(r.id); }}
                 onClick={() => setSel(cur => cur === r.id ? null : r.id)}
                 onDragOver={e => { if (linkMode && dragFrom && dragFrom !== r.id) e.preventDefault(); }}
                 onDragEnter={e => { if (linkMode && dragFrom && dragFrom !== r.id) setDragOver(r.id); }}
                 ...
                 onDrop={async (e) => {
                   ...
                   try {
                     if (linkAction === 'transport') {
                       window.location.assign(`/functor/transport?from=${fromId}&to=${toId}`);
                     } else {
                       await createLink(linkAction, fromId, toId);
                       alert(`Created ${linkAction} link`);
                     }
                   } catch (err:any) {
                     console.error(err);
                     alert('Link failed');
            ...
            >
              ...
              <div
                draggable={linkMode}
                onDragStart={e => { if (!linkMode) return; setDragFrom(r.id); e.dataTransfer.setData('text/plain', r.id); }}
                onDragEnd={() => { setDragFrom(null); setDragOver(null); }}
                title={linkMode ? 'Drag to another room to create a link / transport' : undefined}
                className={clsx(linkMode && 'cursor-grab active:cursor-grabbing')}
                aria-label={linkMode ? 'Drag to create link' : undefined}
              >
                <Donut r={18} acc={acc} rej={rej} und={und} tauShare={gated} selected={selected} />
              </div>

              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate" title={r.title ?? r.id}>
                  {r.title ?? `room:${r.id.slice(0,6)}…`}
                </div>
                <div className="text-[11px] text-slate-600">
                  args {r.nArgs} • edges {r.nEdges} {r.updatedAt && <>• {new Date(r.updatedAt).toLocaleDateString()}</>}
                </div>

                {(r.tags?.length ? (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {r.tags!.slice(0,4).map(t => (
                      <span key={t} className="text-[10px] px-1 rounded border bg-white">{t}</span>
                    ))}
                    {r.tags!.length>4 && <span className="text-[10px] text-slate-500">+{r.tags!.length-4}</span>}
                  </div>
                ) : null)}

                <div className="mt-1 flex flex-wrap gap-1.5">
                  <Chip color={KCOL.xref}         label={KLBL.xref}         value={c.xref} />
                  <Chip color={KCOL.overlap}      label={KLBL.overlap}      value={c.overlap} />
                  <Chip color={KCOL.stack_ref}    label={KLBL.stack_ref}    value={c.stack_ref} />
                  <Chip color={KCOL.imports}      label={KLBL.imports}      value={c.imports} />
                  <Chip color={KCOL.shared_author}label={KLBL.shared_author}value={c.shared_author} />
                </div>

                <div className="mt-2 flex gap-1">
                  <button className="px-1.5 py-0.5 rounded border text-[12px] hover:bg-slate-50"
                          onClick={(e)=>{ e.stopPropagation(); window.location.assign(`/deliberation/${r.id}`); }}>
                    open
                  </button>
                  <button className="px-1.5 py-0.5 rounded border text-[12px] hover:bg-slate-50"
                          onClick={(e)=>{ e.stopPropagation(); window.location.assign(`/sheets/delib:${r.id}`); }}>
                    sheet
                  </button>
                  {source && source!==r.id ? (
                    <button className="px-1.5 py-0.5 rounded border text-[12px] hover:bg-slate-50"
        ...
        return (
          <div className="mt-3 rounded-lg border bg-white/80 p-3">
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold">Neighborhood • {me.title ?? me.id.slice(0,6)+'…'}</div>
              <button className="text-[11px] underline" onClick={()=>setSel(null)}>Close</button>
            </div>
            <ul className="mt-2 grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {nbrs.map(n => {
                ...
                const acc=r.accepted/t, rej=r.rejected/t, und=r.undecided/t;
                return (
                  <li key={r.id} className="rounded border p-2 flex items-center gap-2">
                    <Donut r={12} acc={acc} rej={rej} und={und} selected={false} />
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-medium truncate">{r.title ?? r.id}</div>
                      <div className="text-[11px] text-slate-600">weight {n.weight}</div>
                      <div className="mt-1 flex gap-1.5">
                        {n.kinds.map(k => <Chip key={k} color={KCOL[k]} label={KLBL[k]} value={(c as any)[k] ?? 0} />)}
                      </div>
                    </div>
                    <button className="px-1.5 py-0.5 rounded border text-[11px] hover:bg-slate-50"
                            onClick={()=>window.location.assign(`/deliberation/${r.id}`)}>open</button>
                  </li>
                );
              })}
              {nbrs.length===0 && <li className="text-[12px] text-slate-600">No immediate neighbors.</li>}
            </ul>
          </div>
        );
      })()}
