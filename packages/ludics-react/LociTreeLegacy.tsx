...
export function LociTreeLegacy({
  root,
  onPickBranch,
  usedAdditive,
  focusPath,
  ...
  const render = (n: LociNode) => {
    ...
    return (
      ...
      >
        <div
          className={[
            "relative grid items-start gap-2 px-2 py-1.5 rounded-xl border border-slate-400 transition",
            // braided 3-column header: [P rail] [center locus+chooser] [O rail]
            "md:grid-cols-[minmax(6rem,1fr)_auto_minmax(6rem,1fr)]",
            // Background color based on polarity
            actsP.length > 0 && actsO.length === 0
              ? "bg-emerald-100/50"
              : actsO.length > 0 && actsP.length === 0
              ? "bg-rose-100/50"
              : "bg-slate-200/50",
        ...
        {additive && suffixes.length > 0 && (
          <div className="mt-1 ml-6 flex flex-wrap gap-1">
            {suffixes.map((suf) => (
              <button
                key={suf}
                onClick={() => onPickBranch?.(n.path, suf)}
                disabled={!!chosen}
                className={[
