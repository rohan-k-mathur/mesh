...
export function isLudicsAutoMintConfigured(): boolean {
  return Boolean(
    process.env.LUDICS_JWT_SIGNING_KEY && process.env.LUDICS_PARTICIPANT_ID,
  );
}
...
function getSigningKey(): Uint8Array {
  ...
  if (!raw) {
    throw new Error(
      "LUDICS_JWT_SIGNING_KEY is not configured on the MCP server; cannot auto-mint scoped tokens.",
    );
  }
 ...
 */
export async function mintScopedToken(deliberationId: string): Promise<string> {
  if (!deliberationId) {
  ...
  if (!participantId) {
    throw new Error(
      "LUDICS_PARTICIPANT_ID is not configured on the MCP server; cannot auto-mint scoped tokens.",
    );
  }
