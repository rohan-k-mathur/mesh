    ...
    .array(
      z.object({
        ...
        evidence: z
          ...
          .default([])
          .describe(
            "Per-premise evidence (up to 5). Attaches to THIS premise's minted Claim row, not the conclusion. Use this when each premise has its own distinct source (e.g. premise about cardiovascular risk → Riley Children's Health link; premise about neurocognitive interference → PLOS link). Ignored on a `reuseClaimId` premise (the existing claim keeps its own evidence). If a premise is deduped (collapses into another premise or the conclusion), its evidence is merged into the surviving claim and a `premise_evidence_merged` warning is emitted.",
          ),
      }),
    )
    .min(1)
    ...
    z
      .object({
        reuseClaimId: z
          ...
          .min(1)
          .describe(
            "Thread this premise onto a PRIOR link's conclusion by claim id — pass the `conclusionClaimId` an earlier link returned. Skips minting and wires the shared Claim directly (fork-proof). Must reference a prior link's conclusion in THIS request and the same deliberation; forward references / cycles → CHAIN_LINK_INVALID_THREAD.",
          ),
        text: z
          .string()
          .max(1000)
          .optional()
          .describe(
            "Optional sanity declaration of the reused claim's text. If supplied and its content hash diverges from the referenced claim, the chain is rejected with CHAIN_LINK_BROKEN (no silent fork).",
          ),
      })
...
const ProposeWarrantInput = z.object({
  ...
  sessionId: z
    ...
    .optional()
    .describe(
      "Per-session capability token, recorded in the warrant argument's provenance. Reuse the SAME UUIDv4 you use for your other writes and answer_critical_question calls so you can later self-canonicalise your own answers to this warrant's critical questions. Never log or surface this token.",
    ),
});
...
const tools: ToolSpec[] = [
  ...
  },
  {
    ...
    async handler(args) {
      ...
      const availableClusterTags = Array.from(
        new Set(
          all
