...
async function main() {
  ...
  // Ensure participants are recorded (some UIs rely on this)
  await prisma.conversationParticipant.upsert({
    where: { conversation_id_user_id: { conversation_id: conv.id, user_id: alice.id } },
    update: {},
    create: { conversation_id: conv.id, user_id: alice.id },
  });
  await prisma.conversationParticipant.upsert({
    where: { conversation_id_user_id: { conversation_id: conv.id, user_id: bob.id } },
    update: {},
    create: { conversation_id: conv.id, user_id: bob.id },
  });
  ...
  await prisma.userRole.upsert({
    where: { userId_role: { userId: alice.id, role: 'MOD' } },
    update: {},
    create: { userId: alice.id, role: 'MOD' },
  });
